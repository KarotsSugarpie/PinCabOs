...
TABLES_ROOT = BASE / "vpinball" / "Tables"
...
def detect_rom_name(root, provided_rom=""):
...
def looks_like_pup_dir(d):
    ...
    if "pinupplayer.ini" in names:
        return True

    if any(x.suffix.lower() == ".pup" for x in files):
        return True

    if "pup" in lname and len(files) >= 2:
        return True
...
def classify_and_install(extract_root, table_dir, rom):
    ...
    # détecter un dossier PuP par PinUpPlayer.ini / .pup.
    if not literal_pupvideos_dirs:
        pup_roots = find_roots(extract_root, looks_like_pup_dir)
        for pup in pup_roots:
            name = best_plugin_folder_name(pup, table_title)
            dest = table_dir / "pupvideos" / name
            installed["pupvideos"].extend(copy_dir_contents(pup, dest))
    ...
    for alt in altsound_roots:
        ...
        dest = table_dir / "altsound" / name
        installed["altsound"].extend(copy_dir_contents(alt, dest))
    ...
    for f in sorted(list_files(extract_root)):
        ...
        if suffix == ".ini":
            ...
            if "altsound" in full:
                dest = copy_file(f, table_dir / "altsound" / safe_name(rom or table_title), f.name)
                installed["altsound"].append(str(dest))
                continue
        ...
        if suffix in VNI_EXTS:
            ...
            installed["vni"].append(str(copy_file(f, table_dir / "vni" / folder, f.name)))
            continue

        if suffix in SERUM_EXTS:
            folder = safe_name(rom or f.stem or table_title)
            installed["serum"].append(str(copy_file(f, table_dir / "serum" / folder, f.name)))
            continue
        ...
        if suffix in AUDIO_EXTS:
            if "music" in full:
                ...
                installed["altsound"].append(str(copy_file(f, table_dir / "altsound" / safe_name(rom or table_title), f.name)))
            else:
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="pincabos-portable-table-import-") as td:
        ...
        rom = detect_rom_name(extract_root, args.rom)
