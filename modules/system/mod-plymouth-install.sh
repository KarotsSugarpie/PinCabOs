#!/usr/bin/env bash
# PINCABOS_SCRIPT_NAME="mod-plymouth-install.sh"
# PINCABOS_SCRIPT_DESCRIPTION="Installe et applique le thème Plymouth d'installation PinCabOS depuis package .tar.zst"
...
# PINCABOS_SCRIPT_REQUIRES_FILES="/opt/pincabos/modules/system/assets/plymouth/pkg-plymouth-install.tar.zst"
...
clear
echo -e "${ORANGE}────────────────────────────────────────────────────────────────${NC}"
echo -e "${ORANGE} PinCabOS - Module Plymouth INSTALL${NC}"
echo -e "${ORANGE}────────────────────────────────────────────────────────────────${NC}"

THEME_NAME="pincabos-install"
PKG="/opt/pincabos/modules/system/assets/plymouth/pkg-plymouth-install.tar.zst"
SHA="$PKG.sha256"
BACKUP_DIR="/opt/pincabos/backups/plymouth-install-$(date +%Y%m%d-%H%M%S)"
...
tar --zstd -tf "$PKG" >/tmp/pco-plymouth-install-module-list.txt 2>/tmp/pco-plymouth-install-module.err
if [ $? -ne 0 ]; then
  cat /tmp/pco-plymouth-install-module.err
  nogo "Package illisible: $PKG"
fi

grep -Eq "^\\./?usr/share/plymouth/themes/$THEME_NAME/$THEME_NAME\\.plymouth$" /tmp/pco-plymouth-install-module-list.txt
if [ $? -ne 0 ]; then
  cat /tmp/pco-plymouth-install-module-list.txt
...
echo
echo -e "${YELLOW}Résumé Plymouth INSTALL${NC}"
...
echo
go "Module Plymouth INSTALL terminé"
